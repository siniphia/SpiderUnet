...
class SavePlotsCallback(tf.keras.callbacks.Callback):
    ...
    def on_test_begin(self, v_db):
        ...
        for elem in self.val_dataset:
            prediction_mask = self.model.predict_on_batch(elem[0])
            iou_list = []
            for batch_num in range(self.batch_size):
                for seq_num in range(self.seq_len):
                    if self.seq_len == 1:
                        ...
                        image = elem[0][batch_num][seq_num]
                        label = tf.argmax(elem[1][batch_num][seq_num], axis=-1)
                        masks = tf.argmax(prediction_mask[batch_num][seq_num], axis=-1)
                    ...
                    imageio.imwrite(os.path.join(self.save_path, str(time.time()) + '.png'), masks)
                    # plt.savefig(os.path.join(self.save_path, '%03d_%.3f.png' % ((batch_num * self.seq_len + seq_num + 1), iou)))
                    # plt.close()
            # break
...
class BinaryFocalLoss(keras.losses.Loss):
    def call(self, y_true, y_pred, gamma=2., alpha=.25):
...
class WeightedDiceScoreLoss(keras.losses.Loss):
    def call(self, y_true, y_pred, smooth=1., pos_weight=.90, gamma=2.):
        ...
        """
        y_true_fg = y_true[..., 1]  # b, t, h, w
        y_pred_fg = y_pred[..., 1]  # b, t, h, w
        y_true_bg = y_true[..., 0]  # b, t, h, w
...
def dcs(y_true, y_pred):
    y_true = tf.argmax(y_true, axis=-1)  # b, t, h, w
    y_pred = tf.argmax(y_pred, axis=-1)  # b, t, h, w
